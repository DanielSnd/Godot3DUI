...
public:
    ...
    BoxContainer3D(bool p_vertical = false);
    ~BoxContainer3D();
};

class HBoxContainer3D : public BoxContainer3D {
    GDCLASS(HBoxContainer3D, BoxContainer3D);

public:
    HBoxContainer3D() : BoxContainer3D(false) { is_fixed = true; layout_axis = AXIS_X; }
};

class VBoxContainer3D : public BoxContainer3D {
    GDCLASS(VBoxContainer3D, BoxContainer3D);

public:
    VBoxContainer3D() : BoxContainer3D(true) { is_fixed = true; layout_axis = AXIS_Y; }
};

class ZBoxContainer3D : public BoxContainer3D {
    GDCLASS(ZBoxContainer3D, BoxContainer3D);

public:
    ZBoxContainer3D() : BoxContainer3D(true) { is_fixed = true; layout_axis = AXIS_Z; }
};

VARIANT_ENUM_CAST(BoxContainer3D::AlignmentMode);
VARIANT_ENUM_CAST(BoxContainer3D::PositioningMethod);
VARIANT_ENUM_CAST(BoxContainer3D::SpaceFillMode);
VARIANT_ENUM_CAST(BoxContainer3D::LayoutAxis);
